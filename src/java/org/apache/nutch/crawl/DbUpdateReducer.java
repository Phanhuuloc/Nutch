...
extends GoraReducer<UrlWithScore, NutchWritable, String, WebPage> {
  ...
  protected void reduce(UrlWithScore key, Iterable<NutchWritable> values,
    ...
    //LOG.info("DbUpdateReducer : keyUrl = " + keyUrl);
    ...
    // TODO put urlId to metadata
    page.putToMetadata(
    		new Utf8("urlId"), 
    		ByteBuffer.wrap( String.valueOf(key.getUrlId()).getBytes())
    		);
    // TODO put sameUrlId to metadata
    page.putToMetadata(
    		new Utf8("sameUrlId"), 
    		ByteBuffer.wrap( String.valueOf(key.getSameUrlId()).getBytes())
    		);
    // TODO put urlStatus to metadata
    page.putToMetadata(
    		new Utf8("urlStatus"), 
    		ByteBuffer.wrap( String.valueOf(key.getUrlStatus()).getBytes())
    		);
