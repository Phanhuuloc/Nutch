...
public class InfoCrawler extends NutchTool implements Tool {
	...
	public Map<String,Object> run(Map<String, Object> args) throws Exception {
		...
		String solrUrlMovie = (String) args.get(Nutch.ARG_SOLR_MOVIE);
		int onePhase = 3;
		if (!parse) onePhase++;
		...
		for (int i = 0;; i++) {
			...
			// TODO Index to movie solr
			if (solrUrlMovie != null) {
				status.put(Nutch.STAT_PHASE, "indexMovie" + i);
				jobRes = AtexpatsUtils.runIndex(MoviesIndexerJob.class, args, AtexpatsConstants.TYPE_MOVIE, this);
				if (jobRes != null) {
					subTools.put("indexMovie", jobRes);
				}
			}
...
/*			// TODO Index to listing solr
			if (solrUrlListing != null) {
				...
				jobRes = runIndex(AtexpatsIndexerJob.class, args, TYPE_LISTING);
				if (jobRes != null) {
			...
			if (solrUrlWebSearch != null) {
				...
				jobRes = runIndex(WebSearchIndexerJob.class, args, TYPE_WEB_SEARCH);
				if (jobRes != null) {
	...
	public int run(String[] args) throws Exception {
		if (args.length == 0) {
			System.out.println("Usage: InfoCrawler (<seedDir> | -continue) [-solrlisting <solrURLListing>] [-solrws <solrURLWebSearch>] [-threads n] [-depth i] [-topN N] [-numTasks N]");
			return -1;
		}
		...
		for (int i = 0; i < args.length; i++) {
			if ("-threads".equals(args[i])) {
				...
				i++;
			} else if ("-solrmovie".equals(args[i])) {
				solrUrlMovie = StringUtils.lowerCase(args[i + 1]);
				i++;
		...
		Map<String,Object> argMap = ToolUtil.toArgMap(
				...
				Nutch.ARG_SOLR_MOVIE, solrUrlMovie,
