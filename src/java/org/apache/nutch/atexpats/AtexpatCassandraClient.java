...
import org.apache.gora.cassandra.store.HectorUtils;
...
public class AtexpatCassandraClient<K, T extends PersistentBase> {
  public static final Logger LOG = LoggerFactory.getLogger(AtexpatCassandraClient.class);
  ...
  public List<Row<K, ByteBuffer, ByteBuffer>> execute(CassandraQuery<K, T> cassandraQuery, String family) {
    String batchIdColumn = "bid";
    if ("p".equals(family)) {
    	batchIdColumn = "pbid";
    }
    String[] columnNames = cassandraQuery.getColumns(family);
    ByteBuffer[] columnNameByteBuffers = new ByteBuffer[columnNames.length + 1];
    for (int i = 0; i < columnNames.length; i++) {
      columnNameByteBuffers[i] = StringSerializer.get().toByteBuffer(columnNames[i]);
    }

    
    //if (AtexpatsConstants.BATCH_ID != null) {
    	columnNameByteBuffers[columnNames.length] = StringSerializer.get().toByteBuffer(batchIdColumn);
    //}
    ...
    if (AtexpatsConstants.BATCH_ID != null) {
    	...
    	rangeSlicesQuery.addEqualsExpression(StringSerializer.get().toByteBuffer(batchIdColumn), StringSerializer.get().toByteBuffer(AtexpatsConstants.BATCH_ID));
    }
    System.out.println("rangeSlicesQuery = " + rangeSlicesQuery.toString());
    System.out.println("Batch_ID = " + AtexpatsConstants.BATCH_ID);
...
//    IndexedSlicesQuery<K, ByteBuffer, ByteBuffer> indexedSliceQuery = HFactory.createIndexedSlicesQuery(this.keyspace, this.keySerializer, ByteBufferSerializer.get(), ByteBufferSerializer.get());
//    indexedSliceQuery.
